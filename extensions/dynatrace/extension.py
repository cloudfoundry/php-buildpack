...
class DynatraceInstaller(object):
    ...
    def _load_service_info(self):
        ...
        if len(detected_services) == 1:
            ...
            self._ctx['DT_NETWORK_ZONE'] = detected_services[0].get('networkzone', None)
    ...
    def adding_ld_preload_settings(self):
        ...
        location = self._ctx.get('DT_NETWORK_ZONE')
        if location:
            extra_env += '\nexport DT_NETWORK_ZONE="${{DT_NETWORK_ZONE:-{}}}"'.format(location)
