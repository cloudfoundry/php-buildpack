...
def preprocess_commands(ctx):
	...
	#}
    	...
    	#" export APP_HOSTNAME=$APP_TIERNAME-`echo $VCAP_APPLICATION | sed -e 's/.*instance_index.://g;s/\".*host.*//g' | sed 's/,//'`; echo APP_NAME $APP_NAME; echo APP_TIERNAME $APP_TIERNAME; echo APP_HOSTNAME $APP_HOSTNAME; export AD_ACCOUNT_NAME=`echo $VCAP_SERVICES | sed -e 's/.*account-name.:.//g;s/\".*port.*//g' `; echo AD_ACCOUNT_NAME $AD_ACCOUNT_NAME; export AD_ACCOUNT_ACCESS_KEY=`echo $VCAP_SERVICES | sed -e 's/.*account-access-key.:.//g;s/\".*host-name.*//g' `; echo AD_ACCOUNT_ACCESS_KEY $AD_ACCOUNT_ACCESS_KEY; export AD_CONTROLLER=`echo $VCAP_SERVICES | sed -e 's/.*host-name.:.//g;s/\".*ssl-enabled.*//g' ` ; echo AD_CONTROLLER $AD_CONTROLLER; export AD_PORT=`echo $VCAP_SERVICES | sed -e 's/.*port.:.//g;s/\".*account-access-key.*//g' ` ; echo AD_PORT $AD_PORT; export sslenabled=`echo $VCAP_SERVICES | sed -e 's/.*ssl-enabled.:.//g;s/\".*.*//g' `; echo sslenabled $sslenabled; if [ $sslenabled == \"true\" ] ; then export sslflag=\-s ; fi;  PATH=$PATH:./app/php/bin/ ./app/appdynamics/appdynamics-php-agent/install.sh $sslflag -i ./app/appdynamics/phpini -a=$AD_ACCOUNT_NAME@$AD_ACCOUNT_ACCESS_KEY $AD_CONTROLLER $AD_PORT $APP_NAME $APP_TIERNAME $APP_HOSTNAME ; cat /home/vcap/app/appdynamics/phpini/appdynamics_agent.ini >> /home/vcap/app/php/etc/php.ini"
	...
	[ 'export', ' sslenabled=`echo $VCAP_SERVICES | sed -e 's/.*ssl-enabled.:.//g;s/\".*.*//g'; if [ $sslenabled == \"true\" ] ; then export sslflag=\-s ; fi; `],
